...
def validate(
    ...
    if (
        provenance.get("targets") != TARGETS
        or float(provenance.get("target_window", -1)) != TARGET_WINDOW
    ):
    ...
    for source in selected_source:
        ...
        if within != (
            float(source["recall_min"]) >= goal
            and recall <= goal + TARGET_WINDOW
        ):
            raise ValueError(
                f"target-window flag disagrees with recall: {workload}/{method}"
            )
        if (
            float(source["filter_violations"]) != 0
            or float(source["sentinel_errors"]) != 0
        ):
            raise ValueError(f"correctness failure: {workload}/{method}")
        if (
            method != "default_cagra"
            and float(source["duplicate_output_query_rate_max"]) != 0
        ):
            raise ValueError(
                f"duplicate Retain/NaviX output: {workload}/{method}"
            )
        status = (
            "matched"
            if within
            else ("nearest_overshoot" if reached else "unreached")
        )
        selected.append(
            {
                "workload": workload,
    ...
    expected = {
        (workload, method) for workload in WORKLOADS for method in METHODS
    }
    ...
    for source in read_csv(root / "analysis/measurements.csv"):
        if (
            source["workload"] not in WORKLOADS
            or source["method"] not in METHODS
        ):
            continue
        measurements.append(
            {
...
def write_csv(path: Path, rows: list[dict[str, object]]) -> None:
    with path.open("w", newline="", encoding="utf-8") as output:
        writer = csv.DictWriter(
            output, fieldnames=TABLE_FIELDS, lineterminator="\n"
        )
        writer.writeheader()
    ...
    for workload in WORKLOADS:
        ...
        for method in METHODS:
            ...
            if row["status"] == "matched":
                cells.append(
                    f"{qps_label(float(row['qps']))} ({float(row['recall']):.4f})"
                )
            elif row["status"] == "nearest_overshoot":
                cells.append(f"-- [nearest {float(row['recall']):.4f}]")
            else:
                cells.append(
                    f"{qps_label(float(row['qps']))} (max {float(row['recall']):.4f})"
                )
        lines.append(
...
def write_plot(path: Path, measurements: list[dict[str, object]]) -> None:
    ...
    handles = []
    labels = []
    for workload_index, (axis, workload) in enumerate(
        zip(axes, WORKLOADS, strict=True)
    ):
    ...
    for extension in ("pdf", "png"):
        figure.savefig(
            path.with_suffix(f".{extension}"), dpi=220, bbox_inches="tight"
        )
    plt.close(figure)
...
def main() -> None:
    ...
    selected.sort(
        key=lambda row: (
            WORKLOADS.index(str(row["workload"])),
            list(METHODS).index(str(row["method"])),
        )
    )
    ...
    print(
        json.dumps(
            {"selected": len(selected), "measurements": len(measurements)}
        )
    )
