...
reference_root=${RETRIEVE_A100_K100_RUN_ROOT:?set RETRIEVE_A100_K100_RUN_ROOT to the completed k=100 run}
result_root=${RETRIEVE_A100_SEED_RUN_ROOT:-"${reference_root}/seed_ablation"}
...
selected=${RETRIEVE_SEED_REFERENCE_SELECTED:-"${reference_root}/matched_recall/analysis/selected_points.csv"}
summary=${RETRIEVE_SEED_REFERENCE_SUMMARY:-"${reference_root}/gpu_graph/analysis/summary_points.csv"}
selected_provenance=${RETRIEVE_SEED_REFERENCE_PROVENANCE:-"${reference_root}/matched_recall/analysis/provenance.json"}
stage=${1:-all}
...
flock -n 9 || { echo "another seed-ablation stage owns ${result_root}.lock" >&2; exit 2; }
...
require_inputs() {
  local paths=(
    ...
    "${data_root}/navix_bitmap_k100/yfcc/correctness_1000/manifest.json"
    "${data_root}/navix_bitmap_k100/yfcc/throughput_10000/manifest.json"
  )
  for path in "${paths[@]}"; do
    test -e "${path}" || { echo "missing seed-ablation input: ${path}" >&2; exit 2; }
  done
}
...
if int(profile["max_queries"]) != 2048:
    raise SystemExit("seed ablation requires max_queries=2048")
spec = profile["datasets"]["yfcc"]
if int(spec["graph_degree"]) != 64:
    raise SystemExit("seed ablation requires the YFCC degree-64 graph")
PY
...
test_gate() {
  env LD_PRELOAD="${libcuvs}${LD_PRELOAD:+:${LD_PRELOAD}}" \
    "${repo_dir}/cpp/build/gtests/NEIGHBORS_ANN_CAGRA_FILTER_BITMAP_TEST" \
      --gtest_filter='*BitmapSeededNavixSupportsSeedCapBelowResultWidth*' \
      --gtest_color=no
  env MPLBACKEND=Agg "${python_bin}" "${script_dir}/test_pipeline.py"
}

initialize() {
  "${python_bin}" "${script_dir}/seed_ablation.py" initialize \
  ...
  "${python_bin}" "${repo_dir}/benchmarks/retrieve_workshop/gpu_graph/capture_provenance.py" \
    --result-root "${result_root}" --repo "${repo_dir}" --stage seed_ablation \
...
PY
  ...
  local phase repetitions min_time raw_dir
...
calibrate() {
  ...
  run_pending
  local round
  for round in $(seq 0 31); do
    "${python_bin}" "${script_dir}/seed_ablation.py" plan-next \
      --root "${result_root}" --data-root "${data_root}"
...
if not state["complete"]:
    raise SystemExit("calibration exceeded 32 adaptive rounds")
PY
}

analyze_preliminary() {
  env MPLBACKEND=Agg "${python_bin}" "${script_dir}/seed_ablation.py" analyze \
    --root "${result_root}"
}

diagnostics() {
  "${python_bin}" "${script_dir}/seed_ablation.py" generate-diagnostics \
    --root "${result_root}" --data-root "${data_root}"
  run_pending
}
...
bundle() {
  "${python_bin}" "${script_dir}/seed_ablation.py" bundle --root "${result_root}"
}
...
case "${stage}" in
  ...
  run) calibrate; analyze_preliminary; diagnostics; analyze_final ;;
  analyze) analyze_final ;;
  bundle) bundle ;;
  all)
    preflight
    build_binaries
